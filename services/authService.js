...
export async function loginWithMartha(email, password) {
  ...
  if (!res.success) throw new Error(res.error || "Erreur Martha (auth)");
...
export async function signupWithMartha(username, email, password) {
  ...
  if (!ins.success) {
    ...
    if (msg.includes("1062"))
      throw new Error("Ce nom d’utilisateur ou cet email est déjà pris.");
    throw new Error(ins.error || "Erreur d'inscription");
  }
